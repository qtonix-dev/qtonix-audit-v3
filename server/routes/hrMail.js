...
const { Settings, HrCandidate, HrJobPost } = require('../models');
...
router.get('/candidates/:id/emails', requireHrAccess, async (req, res, next) => {
  try {
    ...
    const msgs = await gmail.searchMessages(s, token, email, `{from:${cand.email} to:${cand.email}}`, 30);
    // Sort oldest → newest for a conversation view.
...
router.post('/candidates/:id/schedule-interview', requireHrAccess, async (req, res, next) => {
  try {
    const { s, token, email } = await hrMailbox();
    if (!token) return res.status(400).json({ error: 'The recruitment mailbox isn’t linked yet. Ask an admin to connect it in HR Admin.' });
    const cand = await HrCandidate.findByPk(req.params.id);
    ...
    const title = b.title || `Interview: ${cand.name}${job ? ` — ${job.title}` : ''}`;
    let event = {};
    try {
      event = await gmail.createCalendarEvent(s, token, {
        ...
        attendees: [cand.email, ...(Array.isArray(b.extraAttendees) ? b.extraAttendees : [])].filter(Boolean),
    ...
    const iv = { id: `iv${Date.now()}`, at: start.toISOString(), mode: b.mode || 'online', notes: b.notes || '', by: req.hrActor.name, meetLink: event.meetLink, eventLink: event.htmlLink };
    const list = Array.isArray(cand.interviews) ? cand.interviews.slice() : [];
    list.unshift(iv);
    cand.interviews = list; cand.changed('interviews', true);
    const t = Array.isArray(cand.timeline) ? cand.timeline.slice() : [];
    t.unshift({ id: `t${Date.now()}`, type: 'interview', text: `Interview scheduled by ${req.hrActor.name} for ${start.toLocaleString()}${event.meetLink ? ' (Google Meet)' : ''}.`, by: req.hrActor.name, at: new Date().toISOString() });
    cand.timeline = t; cand.changed('timeline', true);
    await cand.save();
...
<p>We'd like to invite you to an interview${job ? ` for the <strong>${job.title}</strong> role` : ''}.</p>

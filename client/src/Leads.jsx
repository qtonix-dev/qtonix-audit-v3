    ...
    <div className="max-w-3xl">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 space-y-5">
        {/* Back-dating, for lead managers and admins migrating historical leads
            ...
            see it. Never shown when adding a call-back — those are always live. */}
...
function EmailDraftTab({ lead, user, onChange }) {
  ...
  const [busy, setBusy] = useState(false);
  const [modal, setModal] = useState(null); // 'first' | 'reminder'
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          {isOwner && !lead.firstReplyDoneAt && !lead.firstDraft && (
        ...
        {lead.firstReplyDoneAt ? (
          ...
          <div className="text-xs text-slate-400">
            {isOwner ? 'No first reply yet. Reply yourself, or add a draft for the lead manager to send.' : 'Waiting on the owner to action the first reply.'}
          </div>
        )}
      </div>
      ...
      <div className="pt-4 border-t border-slate-100">
        ...
        {lead.reminderReceived ? (
          ...
          <div className="text-xs text-slate-400">{isOwner ? 'No reminder yet. Add one for the lead manager to send on your behalf.' : 'No reminder submitted yet.'}</div>
        )}
      </div>

      {modal && (
        <DraftModal
          title={modal === 'first' ? 'First reply' : 'Reminder'}
          busy={busy}
          onClose={() => setModal(null)}
          onSubmit={async (subject, body) => {
            await save(modal === 'first' ? 'first-reply' : 'reminder-draft', modal === 'first' ? { draft: body, subject } : { draft: body, subject });
            setModal(null);
          }} />
      )}
    </div>
...
function SubmittedDraft({ subject, body, at, canAck, ackLabel, onAck, busy }) {
  return (
...
function DraftModal({ title, onClose, onSubmit, busy }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl p-6 w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
        <div className="text-base font-extrabold text-[#050A1F] mb-1">{title} draft</div>
        ...
        <div className="flex gap-2 mt-4">
          <button disabled={busy || !bodyText.trim()} onClick={() => onSubmit(subject, body)}
            ...
            style={{ background: 'linear-gradient(90deg,#FF6A00,#FF4500)' }}>
            {busy ? 'Submitting…' : 'Submit to lead manager'}
          </button>
